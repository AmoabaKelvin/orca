...
export function getPiSubagentSnapshotSourceLines(): string[] {
  return [
    'type SubagentDetail = { agentType?: string; description?: string; startedAt: number }',
    ...
    '    if (subagentRoster.exited?.has(id)) continue',
...
export function getPiSubagentRosterEventSourceLines(): string[] {
  return [
    ...
    '    const record = event as { id?: unknown; runId?: unknown; agent?: unknown; description?: unknown }',
    ...
    '      if (!subagentDetails.has(id)) subagentDetails.set(id, { agentType: readLabel(record.agent), description: readLabel(record.description), startedAt: Date.now() })',
    "      post('agent_start')",
    '      return',
    '    }',
    "    if (status !== 'completed' && status !== 'failed' && status !== 'aborted') return",
    // Why: a runner exit already dropped the child from the posted roster.
    '    const wasVisible = lifecycleState.active.has(id) && !lifecycleState.exited?.has(id)',
    ...
    '    const newlyExited = !lifecycleState.exited.has(runId)',
    '    lifecycleState.exited.add(runId)',
    '    if (newlyExited) postSubagentsUpdate()',
